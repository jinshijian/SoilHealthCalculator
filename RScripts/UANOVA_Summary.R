...
# for (i in var_ID){
  ...
  layer1Perc <- length(subdata[!is.na(subdata$mean_Climate),1])/448*100
  layer2Perc <- length(subdata[!is.na(subdata$mean_GrainCrop),1])/448*100
  layer3Perc <- length(subdata[!is.na(subdata$mean_CoverCrop),1])/448*100
  layer4Perc <- length(subdata[!is.na(subdata$mean_Texture),1])/448*100
